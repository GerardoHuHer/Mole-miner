...
# ─────────────────────────────────────────────────────────────────────────────
#  Pantalla de controles — mismo fondo de mina que el menú principal
# ─────────────────────────────────────────────────────────────────────────────
...
const ORES := [
	[Color(0.098, 0.098, 0.098, 1.0), 7,  6271, 2381],
	[Color(0.816, 0.682, 0.580, 1.0), 16, 4733, 1847],
	[Color(0.980, 0.851, 0.259, 1.0), 38, 3571, 2143],
	[Color(0.141, 0.341, 0.710, 1.0), 52, 5113, 3001],
	[Color(0.859, 0.082, 0.082, 1.0), 58, 2803, 4127],
	[Color(0.322, 0.882, 0.941, 1.0), 78, 7177, 1993],
	[Color(0.059, 0.890, 0.376, 1.0), 88, 6899, 3217],
	[Color(0.780, 0.400, 0.180, 1.0), 44, 3923, 5501],
]
...
# ─────────────────────────────────────────────────────────────────────────────
#  FONDO DE MINA (idéntico al menú principal)
# ─────────────────────────────────────────────────────────────────────────────
func _draw() -> void:
	var w := size.x
	var h := size.y
...
func _tile_color(row, col, total_rows, grass_end, dirt_end, gravel_end) -> Color:
	...
	if row < grass_end:  return GRASS_C[ha % GRASS_C.size()]
	if row < dirt_end:   return DIRT_C[ha % DIRT_C.size()]
	if row < gravel_end: return GRAVEL_C[ha % GRAVEL_C.size()]
...
func _draw_roots(cols: int, grass_end: int, dirt_end: int) -> void:
	for col in range(cols):
		if (col * 3 + 7) % 5 != 0: continue
		var start_row := grass_end + 1
		var length    := 2 + ((col * 17 + 3) % 3)
		for r in range(start_row, min(start_row + length, dirt_end)):
			draw_rect(Rect2(col * TILE + TILE * 0.35, r * TILE, TILE * 0.30, TILE), ROOT_C)
	...
	for r in range(-rows_r, rows_r + 1):
		for c in range(-cols_r, cols_r + 1):
			...
			if nx * nx + ny * ny * 0.75 <= 1.0:
				...
				draw_rect(Rect2(cx + c * snap - snap * 0.5, cy + r * snap - snap * 0.5, snap, snap),
					Color(0.01, 0.01, 0.02, alpha))
...
func _draw_torch(x: float, y: float, phase: float) -> void:
	...
	draw_rect(Rect2(x - 4, y - 8,  8, 24), Color(0.314, 0.188, 0.063, 1.0))
	draw_rect(Rect2(x - 3, y - 8,  6, 24), Color(0.451, 0.275, 0.094, 1.0))
